...
Contacts.prototype = {
  ...
  /**
   * Helper method to assure that a button is successfuly clicked. For some
   *  strange reason we are getting intermittent failures due to some button
   *  clicks being successfully delivered but no effect caused (see bug
   *  982260).
   * @param {Marionette.Element} element The element to assure simulate the
   *  click on.
   */
  assureClickOn: function (element) {
    var form = this.client.findElement(Contacts.Selectors.form),
      currentFormY = form.location().y,
      isFormMoving = function() {
        if (currentFormY !== form.location().y) {
          return true;
        } else {
          if (element.enabled()) {
            element.click();
    ...
    this.client.waitFor(isFormMoving);
  },
