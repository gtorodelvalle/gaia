...
Contacts.prototype = {
  ...
  enterContactDetails: function(details) {
    ...
    for (var i in details) {
      ...
      var formTitle = this.client.helper.waitForElement(selectors.formTitle);
      formTitle.tap();
    }

    var save = this.client.helper.waitForElement(selectors.formSave);
    var form = this.client.helper.waitForElement(selectors.form);
    var formLocationY = form.location().y;
    var isFormMoving = function() {
      if (formLocationY !== form.location().y) {
        return true;
      } else if (save.enabled()) {
        this.clickOn(save);
      }
    };
    this.client.waitFor(isFormMoving.bind(this));
